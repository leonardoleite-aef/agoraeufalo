...
- **A Regra Canônica de "Zero Traduções Óbvias":** Proibição absoluta de traduzir números universais (ex: `1973`), dias isolados ou palavras cujo significado visual é óbvio no mundo inteiro. Traduções aplicam-se **SOMENTE** a frases/expressões contraintuitivas, idiomáticas ou onde a lógica do inglês diverge do português (ex: *"a quarter to five"* ➔ *"Quinze pras cinco"*, *"on May 4th"* ➔ *"No dia 4 de maio"*). Tabelas e materiais devem focar no **Bloco Sonoro (Melodia)** e no **Ponto de Confusão do Brasileiro**.
- **Os 2 Grandes Arquétipos Pedagógicos de Cursos:**
  1. **Masterclasses Clássicas / Cursos Temáticos (ex: Dates & Times, QuickStart, Gramática Viva):** Estruturados em Explicação Viva do Sentimento da Estrutura, Tabelas de Melodia Sonora & Pontos Contraintuitivos, Casos de Uso Reais, Exercícios Pautados e Sacada de Ouro do Leo.
  2. **Magic Stories (Método Canônico Narrativo):** Estruturado estritamente nas 6 arenas de treino auditivo e oral (LR, VOC, LA, LRT, LASK, PRO).
...
- **Arquitetura de Assinatura & Checkouts (Hotmart Primário, Stripe Fallback):** O AEF Club possui uma assinatura única (com diferentes tiers/acessos). A assinatura do Club, checkout principal, recuperação de clientes e cobranças recorrentes são processados prioritariamente na **Hotmart** (BRL, PIX, 12x no cartão). O **Stripe** é mantido como **fallback global** para clientes internacionais, moedas estrangeiras (USD/EUR) ou transações alternativas. Cursos também podem ser configurados com Venda Avulsa (standalone) ou Degustação Free modular.
...
3. **`3. Listen & Answer (LA)` (Reflexo & Velocidade de Resposta no Diálogo):**
   ...
   - **Regra Estrita de Magic Stories (Zero Respostas Reveladas):** **NÃO MOSTRAR AS RESPOSTAS!** Tanto na apostila/PDF de treino quanto na Sala de Aula, exibir apenas a **Pergunta** com linhas pautadas/espaço em branco para resposta manual ou reflexo oral. A resposta pronta é proibida para eliminar muletas visuais.
   - Cards intercalados no Player: Pergunta no áudio com micro-pausa de 2 a 4 segundos antes da confirmação.
4. **`4. Look & Retell (LRT)` (Produção Própria & Speaking Ativo com Guia Visual):**
   ...
   - **Zero áudio prévio e zero auto-scroll.** 100% Speaking autônomo. O aluno utiliza um **estímulo/guia visual** para recontar a história com suas próprias palavras.
   - **Os 2 Formatos Oficiais de Estímulo Visual Suportados:**
     1. **As Perguntas-Guia de LA:** As perguntas-guia visuais **SÃO ESTRITAMENTE AS MESMAS PERGUNTAS DE LISTEN & ANSWER (LA)** (proibição absoluta de inventar perguntas acadêmicas como *"Describe..."* ou *"Tell me about..."*).
     2. **O Filme da Magic Story sem Áudio (quando disponível):** O vídeo mudo da história passando na tela para o aluno narrar/recontar em tempo real com sua própria fala.
   - *(Nota: AI Speech Coach permanece no roadmap futuro, sem necessidade canônica nesta fase).*
...
6. **`6. Pronunciation & Connected Speech (PRO)` (Musicalidade, Boca & Ritmo Mecânico):**
   ...
   - **Regra Estrita de Magic Stories (Texto Completo de LR + Linking Sounds):** A seção de Pronúncia na apostila/PDF e na Sala de Aula deve conter **O TEXTO COMPLETO DO LISTEN & READ (LR)** com as marcações visuais de conexões consoante-vogal (*Linking Sounds*) em destaque azul cobalto, chave fonética das reduções principais, treino de repetição em loop contínuo (`🔂`) e a **Sacada de Ouro do Professor Leo** em destaque monumental âmbar.
...
- **Etapa F (Listen & Ask - Desafio de Perguntas sem Spoilers):** Criar frases de estímulo (afirmativas/negativas) com espaço pautado para o aluno formular a pergunta no reflexo (**sem mostrar a pergunta pronta impressa**) ➔ Síntese Gemini TTS ➔ Gera a seção de Listen & Ask no PDF (LASK) com espaço pautado.
- **Etapa G (Pronunciation & Connected Speech com Texto Integral):** Inserir **o texto integral da história (LR)** com as marcações visuais de conexões consoante-vogal (*Linking Sounds*), chave fonética e ações em cadeia usando o áudio da história (LR) com cortes por sentença, pausas automáticas e botão de loop (`🔂`) ➔ Redigir a Sacada de Ouro do Leo ➔ Gera a seção de Pronunciation Practice no PDF (PRO).
- **Etapa H (Compilação do Livro Impresso no Padrão dos 3 Arquétipos — SEM LIMITE ARTIFICIAL DE PÁGINAS):**
  * **REGRA ABSOLUTA DE EXTENSÃO:** **NÃO EXISTE LIMITE DE NÚMERO DE PÁGINAS.** Cada atividade ocupa exatamente quantas páginas forem necessárias para um resultado de qualidade superior. Se o módulo exigir 30 páginas, serão 30 páginas; se exigir 4 páginas completas, so be it! O critério mandatório é: **TODAS AS PÁGINAS DEVEM ESTAR COM MAIS DE 70% DE PREENCHIMENTO VISUAL/PEDAGÓGICO ÚTIL** (sem páginas vazias, órfãs ou comprimidas artificialmente).
  * Compilar o PDF Oficial em `Material-PDF/[codigo]_[slug]_Apostila_Oficial.pdf` (ReportLab) estruturado nos 3 Arquétipos com fontes confortáveis e generosas:
    ...
    - **Practice Workbook (Arquétipo 3):** Listen & Answer completo com espaço pautado (sem respostas impressas), Look & Retell com perguntas-guia de LA e keywords, Listen & Ask completo com espaço pautado (sem perguntas impressas), Pronunciation Practice com texto integral/linking e a Sacada de Ouro monumental do Leo.
